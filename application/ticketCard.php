            ...
            <section class="services p-0 m-0 mt-4">
                ...
                <div class="progress mt-4">
                        ...
                        <div class="row">
                            <div class="col-sm">
                                <div class="form-group">
                                    ...
                                    <select name="category" class="form-control" id="exampleFormControlSelect1">
                                        <?php 
                                            $category = $fTicket['category'];
                                            $sqlIdCategory = "SELECT * FROM ticket_category WHERE category = '$category'";
                                            $queryIdCategory = mysqli_query($conn, $sqlIdCategory)or die($sqlIdCategory);
                                            $fIdCat = mysqli_fetch_assoc($queryIdCategory);
                                        ?>
                                        <option value="<?php echo $fIdCat['id'];?>" selected>Set: <?php echo $fTicket['category'];?></option>
                                        <?php 
                            ...
                            <div class="col-sm">
                                <div class="form-group">
                                    ...
                                    <?php 
                                        ...
                                        $sqlIdpriority = "SELECT * FROM ticket_priority WHERE priority = '$priority'";
                                        $queryIdpriority = mysqli_query($conn, $sqlIdpriority)or die($sqlIdpriority);
                                        $fIdpriority = mysqli_fetch_assoc($queryIdpriority);
                                    ?>
                                    <option value="<?php echo $fIdpriority['id'];?>" selected>Set: <?php echo $fTicket['priority'];?></option>
                                    <?php 
                                        $sqlPrio = "SELECT * FROM ticket_priority";
                                        $queryPrio = mysqli_query($conn, $sqlPrio)or die($sqlPrio);
                                        $totalPrio = mysqli_num_rows($queryPrio);
                                        if($totalPrio > 0){
                                            while($fPrio = mysqli_fetch_assoc($queryPrio)){?>
                                            <option value="<?php echo $fPrio['id'];?>"><?php echo $fPrio['priority'];?></option>
                                        <?php } } ?>
                                    </select>
                                </div>
                            </div>
                            <div class="col-sm">
                                <div class="form-group">
                                    ...
                                    ?>
                                    <option value="<?php echo $owner ;?>" selected>Set: <?php echo $fOwner['first_name'];?> <?php echo $fOwner['last_name'];?></option>
                                    <?php
                                        $sqlMembers = "SELECT * FROM v_team_members WHERE idTeam = $idTeam AND level IN ('Admin','Tech')";
                                        $queryMembers = mysqli_query($conn, $sqlMembers)or die($sqlMembers);
                                        $totalMembers = mysqli_num_rows($queryMembers);

                                        if($totalMembers > 0){
                                            while($fMembers = mysqli_fetch_assoc($queryMembers)){?>
                                                <option value="<?php echo $fMembers['id'];?>"><?php echo $fMembers['first_name'];?> <?php echo $fMembers['last_name'];?> - <?php echo $fMembers['level'];?></option>
                                        <?php } } ?>
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="comment" class="form-control" style="min-height: 150px" id="exampleFormControlTextarea1" rows="3"><?php echo $fIdOwner['comment'];?></textarea>
                    </div>
                    <div class="container pt-1 mt-2 pl-0 pr-0">
                        <div class="row">
                            <div class="col">
                        ...
                        <div class="row">
                            ...
                            <?php 
                                ...
                                if($idStatus == 1 || $idStatus == 2 ){?>
                                    <a type="button" class="btn btn-primary" href="teamPipeline.php?idTeam=<?php echo $idTeam;?>">BACK</a>
                                    <button type="submit" class="btn btn-success" name="saveTicket">SAVE</button>
                                    <button class="btn btn-danger" name="closeTicket">CLOSE</button>
