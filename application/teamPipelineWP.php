        ...
        <div id="main">
          <div class="inner">
            ...
            <ul class="nav nav-tabs">
                <li class="nav-item">
                    <a class="nav-link" href="teamPipeline.php?idTeam=<?php echo $idTeam;?>">Backlog</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link active" aria-current="page" href="#">Working Progress</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="teamPipelineCl.php?idTeam=<?php echo $idTeam;?>">Closed</a>
                </li>
            </ul>
            
            <!-- Show ALL ticket to ADMNIN that are in -->
            <?php 
                $sqlTicket ="SELECT * FROM ticket, ticket_info_status WHERE idTeam = $idTeam AND idStatus = 2";
                $queryTicket = mysqli_query($conn, $sqlTicket)or die($sqlTicket);
                $fidTicket = mysqli_fetch_assoc($queryTicket);

                $sqlTicketCard = "SELECT * FROM v_ticket_card WHERE status = 'Working Progress' AND idTeam = $idTeam ORDER BY idTicket ASC";
